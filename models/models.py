...
from .snp_cot_fusion import SNPFusion
...
class Network(nn.Module):
    def __init__(self, cfg):
        ...
        self.attention = SNPFusion(5, 5)
        # CotA
        # self.attention = CoTAttention(5,5)
    ...
    def load_checkpoint(self):
        if isfile(self.cfg.resume):
            ...
            checkpoint = torch.load(self.cfg.resume, map_location=torch.device('cpu'))
    ...
    def forward(self, x):
        ...
        results = [so1, so2, so3, so4, so5]  # side output
