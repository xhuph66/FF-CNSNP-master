...
def main():
    ...
    if args.mode == "test":
        ...
        # log
        log = Logger(join(TMP_DIR, "%s-%d-log-bsds.txt" % ("sgd", cfg.lr)))
        sys.stdout = log
        ...
        for epoch in range(0, cfg.max_epoch):
            tr_avg_loss, tr_detail_loss = train(cfg,
                                                ...
                                                save_dir=join(TMP_DIR, "train-bsds", "epoch-%d-training-record-bsds" % epoch))

            test(cfg, model, test_loader, save_dir=join(TMP_DIR, "train-bsds", "epoch-%d-testing-record-bsds-view" % epoch))
